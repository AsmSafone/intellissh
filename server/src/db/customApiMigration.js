  ...
  try {
    ...
    if (!customApiUrlSetting) {
      ...
      await db.run(
        ...
        ['custom_api_url', 'Custom API URL', '', 'llm', 'Base URL for custom OpenAI-compatible API', false]
      );
    } else {
    ...
    if (!customApiKeySetting) {
      ...
      await db.run(
        'INSERT INTO settings (id, name, value, category, description, is_sensitive) VALUES (?, ?, ?, ?, ?, ?)',
        ['custom_api_key', 'Custom API Key', '', 'llm', 'API key for custom OpenAI-compatible API', true]
      );
    } else {
    ...
    if (!customModelSetting) {
      ...
      await db.run(
        'INSERT INTO settings (id, name, value, category, description, is_sensitive) VALUES (?, ?, ?, ?, ?, ?)',
        ['custom_model', 'Custom Model', 'gpt-3.5-turbo', 'llm', 'Model name for custom API', false]
      );
    } else {
