...
async function runMigration() {
  try {
    // Run new credentials table migration
    ...
    let consoleSnapshotExists = false;

    if (db.type === 'postgres') {
      const result = await db.all("SELECT column_name FROM information_schema.columns WHERE table_name='sessions' AND column_name='console_snapshot'");
      consoleSnapshotExists = result.length > 0;
    } else {
      const tableInfo = await db.all("PRAGMA table_info(sessions)");
      consoleSnapshotExists = tableInfo.some(column => column.name === 'console_snapshot');
    }
    ...
    if (db.type === 'postgres') {
      const result = await db.all("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'settings'");
      settingsTableExists = result.length > 0;
    } else {
      const tablesResult = await db.all("SELECT name FROM sqlite_master WHERE type='table' AND name='settings'");
      settingsTableExists = tablesResult.length > 0;
    }
    ...
    if (db.type === 'postgres') {
      const result = await db.all("SELECT column_name FROM information_schema.columns WHERE table_name='users' AND column_name='role'");
      roleColumnExists = result.length > 0;
    } else {
      const usersTableInfo = await db.all("PRAGMA table_info(users)");
      roleColumnExists = usersTableInfo.some(column => column.name === 'role');
    }
    ...
    if (db.type === 'postgres') {
      const result = await db.all("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_settings'");
      userSettingsTableExists = result.length > 0;
    } else {
      const userSettingsTableResult = await db.all("SELECT name FROM sqlite_master WHERE type='table' AND name='user_settings'");
      userSettingsTableExists = userSettingsTableResult.length > 0;
    }
